...
public class QuantumChannel
{
    ...
    private static String AliceFinalBits = "-1";
    private static String BobFinalBits = "-1";
    private static String EveFinalBits = "-1";

    private static int photonsSent = 0;
    ...
    public static synchronized void forwardTo( Message message, Identity sender ) throws IOException, ClassNotFoundException, InterruptedException
    {
        ...
        for ( int i = 0; i < ActiveParties.size(); i++ )
        {
            if ( ActiveParties.get( i ).name.equals( name ) )
            {
                ...
                if ( EveConnected && !sender.equals( Identity.Eve ) )
                {

                    for ( ServerThread currentThread : ActiveParties )
                    {
                        if ( currentThread.name.equals( Identity.Eve ) )
                        {
                            currentThread.send( message );
                        }
                    }
                }
                if ( message.message instanceof String || !sender.equals( Identity.Eve ) )
                {
                    if ( message.message instanceof photon )
                    {
                        photonsSent++;
                    }
                    if ( name.equals( Identity.Bob ) )
                    {
                        ActiveParties.get( i ).send( message );
                    }
    ...
    static void doFinalStats( Identity name, String bits ) throws InterruptedException, IOException, ClassNotFoundException
    {

        if ( name.equals( Identity.Alice ) && AliceFinalBits.equals( "-1" ) )
        {
            AliceFinalBits = bits;
        }
        if ( name.equals( Identity.Bob ) && BobFinalBits.equals( "-1" ) )
        {
            BobFinalBits = bits;
        }
        if ( name.equals( Identity.Eve ) && EveFinalBits.equals( "-1" ) )
        {
            EveFinalBits = bits;
        }
...
class ServerThread extends Thread
{
    ...
    public void run()
    {
        ...
        try
        {
            ...
            {
                Thread.sleep( 300 );
                if ( QuantumChannel.AliceConnected && QuantumChannel.BobConnected )
                {
                    break;
                }
            }
            ...
            while ( true )
            {
                ...
                } catch ( IOException e )
                {
                    ...
                    if ( name.equals( Identity.Alice ) )
                    {
                        QuantumChannel.AliceConnected = false;
                    }
                    if ( name.equals( Identity.Bob ) )
                    {
                        QuantumChannel.BobConnected = false;
                    }
                    if ( name.equals( Identity.Eve ) )
                    {
                        QuantumChannel.EveConnected = false;
                    }
