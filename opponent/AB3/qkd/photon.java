...
{
    // Density Matrix
    private double[][] state = new double[ 2 ][ 2 ];
    Boolean present;
    private Random rand;
    ...
    public boolean detect()
    {
        ...
        {
            present = false;
            return rand.nextDouble() < config.efficiency;
        } else
        {
            return rand.nextDouble() < config.darkChance;
        }
    }
    ...
    public static String randomBits( int n )
    {
        ...
        for ( int i = 0; i < n; i++ )
        {
            bits.append( Integer.toString( rand.nextInt( 2 ) ) );
        }
